...
void findRotations_triangles(const Eigen::MatrixXd& V0,
    const Eigen::MatrixXd& V1,
    const Eigen::MatrixXi& F,
    const Eigen::MatrixXd& C,
    const std::vector<std::vector<Edge_tr>>& edgeSets,
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < 3; ++j) {//go over all edges surrounding faces
            ...
            const Eigen::Matrix3d r = C(i, j) * e0 * e1.transpose();//weigh by cotangent weight
...
void trianglesEdges(Eigen::MatrixXd& V, Eigen::MatrixXi& F, Eigen::SparseMatrix<double>& L, std::vector<std::vector<Edge_tr>>& edgeSets) {
...
Eigen::MatrixXd rhs_triangles(const Eigen::MatrixXd& C,
    ...
    const Eigen::MatrixXi& F,
    const Eigen::MatrixXd& LV0,
    const std::vector<std::vector<Edge_tr>>& eSets,
    const Eigen::SparseMatrix<double>& L,
    ...
    for (int i = 0; i < nv; ++i) {//go over vertices
        ...
        for (auto& e : eSets[i]) {//go over triangles/edges around vertex
            b += e.w * R[e.rid] * (V.row(e.vertex_pos) - V.row(e.vertex_neg)).transpose();
            check_sum += e.w;
        }
    ...
    if (lambda) {//lambda
        ...
        for (int i = 0; i < nv; ++i) {//go over vertices
            int count = 0;
            for (auto& e : eSets[i]) {//go over triangles/edges around vertex
                //b2.row(i) += (1.0/cvi(i))*R[e.rid] * (LV0.row(i).transpose()); //deleted the 2* as every triangle should be in there twice anyways
                b2.row(i) += (1.0 / 2*cvi(i)) * R[e.rid] * (LV0.row(i).transpose());
                count++;
            }
        }
...
//builds rhs of the equation to be solved, dependent on rotations
...
void findRotations2(const Eigen::MatrixXd& V0,
    const Eigen::MatrixXd& V1,
    const Eigen::MatrixXi& F,
    const Eigen::MatrixXd& C,
    const std::vector<std::vector<Edge_sr>>& edgeSets,
    const Eigen::SparseMatrix<double>& L,
    const Eigen::MatrixXd& LV0,
    const double lambda,
    std::vector<Eigen::Matrix3d>& rot) {
...
void assembleRHS2(const Eigen::MatrixXd& C,
    const Eigen::MatrixXd& V,
    const Eigen::MatrixXi& F,
    const Eigen::MatrixXd& LV0,
    const std::vector<std::vector<Edge_sr>>& eSets,
    ...
    if (lambda) {//lambda
        ...
        for (int i = 0; i < nv; ++i) {//go over vertices
            ...
            for (auto& e : eSets[i]) {//go over triangles/edges around vertex
                //b2.row(i) += (1.0/cvi(i))*R[e.rid] * (LV0.row(i).transpose()); //deleted the 2* as every triangle should be in there twice anyways
                b2.row(i) += R[e.rid] * (LV0.row(i).transpose());
                //std::cout << (R[e.rid] - Eigen::MatrixXd::Identity(3, 3)).norm() << std::endl;
                count++;
            }
            //std::cout << "cvi is " << cvi(i) << " count is "<< count << std::endl;
            b2.row(i) = 2*b2.row(i) / count;
            //b2.row(i) = R[i] * LV0.row(i).transpose();
        }

        rhs += lambda * L * b2;
        std::cout << b2.row(0) << " vs. lhs " << 2 * LV0.row(0) << std::endl;
    }
}
...
void findRotations_spokes(const Eigen::MatrixXd& V0,
    ...
    for (int i = 0; i < m; ++i) {//go over faces
        for (int j = 0; j < 3; ++j) {//go over all edges surrounding faces
            ...
            const Eigen::Vector3d e0 = V0.row(F(i, k1)) - V0.row(F(i, k2));//edge from face i k1 to k2 (goes over all edges due to j)
            const Eigen::Vector3d e1 = V1.row(F(i, k1)) - V1.row(F(i, k2));//same in current setting
            const Eigen::Matrix3d r = C(i, j) * e0 * e1.transpose();//weigh by cotangent weight of face
...
void rhs_spokes(double lambda, MatrixXd& V_orig, MatrixXd&Cov, const std::vector<Eigen::Matrix3d>& R, MatrixXd& rhs) {
    Eigen::MatrixXd b2;
...
void experiment(string mesh) {
    ...
    if (mesh.compare("square_21_spikes.off")==0) {//spiked plane lifting
    ...
    else if (mesh.compare("../cactus_lap.off") == 0) {
        MatrixXd Vorig;
        igl::read_triangle_mesh("../cactus.off", Vorig, F);
        Matrix3d def_r;
        def_r << 0.999641, 0.021031, 0.016579,
            -0.022776, 0.342021, 0.939416,
            0.014087, -0.939457, 0.342378;
        ...
        for (int i = 0; i < fixed.size(); i++) {
            B.push_back(fixed[i]);
            constrPositions.push_back(Vorig.row(fixed[i]));
        }
        for (int i = 0; i < handle.size(); i++) {
            B.push_back(handle[i]);
    ...
    else if (mesh.compare("../bar_lap.off") == 0) {
        MatrixXd Vorig;
        igl::read_triangle_mesh("../bar.off", Vorig, F);
        Matrix3d def_r;
        def_r << -0.707107, -0.707107, 0.0,
            0.707107, -0.707107, 0.0,
            0.0, 0.0, 1.0;
    ...
    else if (mesh.compare("../cylinder_lap.off") == 0) {
        MatrixXd Vorig;
        igl::read_triangle_mesh("../cylinder.off", Vorig, F);
        Matrix3d def_r;
        def_r << -0.5, 0.0, -0.866025,
            0.0, 1.0, 0.0,
            0.866025, 0.0, -0.5;
        ...
        for (int i = 0; i < V.rows(); i++) {
            if (V(i, 0) == min_x || V(i, 0) == max_x || V(i, 1) == min_y || V(i, 1) == max_y) {
                B.push_back(i);
                constrPositions.push_back(V.row(i));
            }
        }
        B.push_back(20202);//middle vertex
        constrPositions.push_back(V.row(20202)+RowVector3d(0, 0, 100));
#endif
    ...
    else if (mesh.compare("../knubbel_lap.off") == 0) {
        MatrixXd Vorig;
        igl::read_triangle_mesh("../knubbel.off", Vorig, F);
        Eigen::MatrixXd def = Eigen::MatrixXd::Zero(1, 3);
        def(0, 0) = -32.299956;//x
        def(0, 2) = 56.703263;//z
        for (int i = 0; i < 1810; i++) {
            B.push_back(i);
            constrPositions.push_back(Vorig.row(i));
        }
        for (int i = 38793; i < V.rows(); i++) {
            B.push_back(i);
        ...
        p(2) += 30;
        constrPositions.push_back(p);//nose
        for (int i = 0; i < V.rows(); i++) {
            if (V(i, 2) < V(4146, 2) - 10) {
                B.push_back(i);
                constrPositions.push_back(V.row(i));
            }
        }
        ...
        B.push_back(3999);
        constrPositions.push_back(V.row(3999));
        B.push_back(3473);
        constrPositions.push_back(V.row(3473));
        B.push_back(6110);//feet
        constrPositions.push_back(V.row(6110));
        B.push_back(777);
        constrPositions.push_back(V.row(777));
        B.push_back(6976);
        constrPositions.push_back(V.row(6976));
        B.push_back(1652);
        constrPositions.push_back(V.row(1652));
        B.push_back(3327);//nose
        Eigen::RowVector3d p = V.row(3327);
        p(0) -= 1;
        p(1) += 2;
        constrPositions.push_back(p);
    }
        ...
        p(1) += 120;
        constrPositions.push_back(p);
        cout << "points are" << endl << 7480 << endl;
        for (int i = 0; i < V.rows(); i++) {
            if (V(i, 1) < height) {
                B.push_back(i);
                constrPositions.push_back(V.row(i));
        ...
        def_r << -1, 0, 0,
            ...
            0, 0.0, -1;
        Vector3d def_t;
        def_t << 5, 0.0, -lower+upper;
        for (int i = 0; i < V.rows(); i++) {
            if (V(i, 2) <= lower+0.002*(upper-lower)) {
                B.push_back(i);
                constrPositions.push_back(V.row(i));
            }
            if (V(i, 2) >= upper-0.002*(upper-lower)) {
                B.push_back(i);
                constrPositions.push_back(def_r*V.row(i).transpose()+def_t);
            }
        }
...
double energy_sr(const MatrixXd& V_orig, const MatrixXd& V_def, std::vector<Eigen::Matrix3d>& R, std::vector<std::vector<Edge_sr>> eSets){
    ...
    findRotations_sr(V_orig, V_def, eSets, R, lambda, false);//ANNIKA NOTE ATTENTION TODO is arap energy now
    energy=0;
    if(1-lambda){//original spokes and rims arap term
        for (auto& edgeSet : eSets) {//go over vertices
            for (auto& e : edgeSet) {//go over triangles/edges around vertex
...
int main(int argc, const char* argv[]) {
    ...
    igl::cotmatrix_entries(V, F, C);
    std::vector<std::vector<Edge_sr>> edgeSets_sr;
    std::vector<std::vector<Edge_tr>> edgeSets_tr;
    spokesAndRimsEdges_sr(V, F, C, edgeSets_sr);
    trianglesEdges(V, F, L, edgeSets_tr);
    mulSparse(L, -1);
    LV0 = L * V;
    LV1 = LV0;
...
/*     A = lambda * L * L + (1.0 - lambda) * L;
    ...
    menu.callback_draw_viewer_menu = [&]()
	{
        ...
        if (ImGui::Button("save .obj and stats", ImVec2(-1, 0)))
        {
            ...
            outFile<<"took "<<metric_time-metric_fac_time<<"s, excluding"<<metric_fac_time<<" for factorization"<<endl;
            ...
            std::cout<<std::endl;
            outFile<<std::endl<<"per-neighbourhood (vertex) non-rigidity (origianal arap) energies"<<std::endl;

            double energy=0;
            int i=0;
            std::vector<Eigen::Matrix3d> rot(V.rows(), Eigen::Matrix3d::Identity());//for triangle
            findRotations_sr(V, V1, edgeSets_sr, rot, lambda,false);
            double min=100;
            double max=-1;
            double avg=0;
            for (auto& edgeSet : edgeSets_sr) {//go over vertices
                energy=0;
                for (auto& e : edgeSet) {//go over triangles/edges around vertex
                    const Eigen::Vector3d e0 = V.row(e.vertex_pos) - V.row(e.vertex_neg);
                    const Eigen::Vector3d e1 = V1.row(e.vertex_pos) - V1.row(e.vertex_neg);
                    energy += e.w * (rot[e.rid] * e0 - e1).squaredNorm();//compute weighted, rotated edge
                }
                i++;
                ...
                avg+=energy/V.rows();//weighted average/ basically original arap energy:)
            }
            outFile<<"min "<<min<<std::endl;
            outFile<<"max "<<max<<std::endl;
            outFile<<"avg "<<avg<<std::endl;
    ...
    doubleFaces << backFaces, F;
    viewer.data().set_mesh(V1, F);//paint mesh. U is V, F faces
    //set shader normals
    Eigen::MatrixXd VN;
    igl::per_vertex_normals(V1, F, VN);
    //MatrixXd doubleNormals(VN.rows() * 2, VN.cols());
    //doubleNormals << VN;
    ...
    //viewer.core().lighting_factor(2.0);
    viewer.launch();
